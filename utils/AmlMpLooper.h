...
#ifndef AML_MP_UTILS_LOOPER_H
#define AML_MP_UTILS_LOOPER_H
 ...
 * can be destroyed.
 */
class MessageHandler : public virtual AmlMpRefBase {
protected:
...
public:
    WeakMessageHandler(const wptr<MessageHandler>& handler);
    virtual void handleMessage(const Message& message);

private:
    wptr<MessageHandler> mHandler;
};
 ...
 * A looper callback.
 */
class LooperCallback : public virtual AmlMpRefBase {
protected:
 ...
 */
class Looper : public AmlMpRefBase {
protected:
    virtual ~Looper();
...
public:
    ...
    int addFd(int fd, int ident, int events, const sptr<LooperCallback>& callback, void* data);
     ...
     * This method can be called on any thread.
     */
    void sendMessage(const sptr<MessageHandler>& handler, const Message& message);
     ...
     * This method can be called on any thread.
     */
    void sendMessageDelayed(nsecs_t uptimeDelay, const sptr<MessageHandler>& handler,
     ...
     * This method can be called on any thread.
     */
    void sendMessageAtTime(nsecs_t uptime, const sptr<MessageHandler>& handler,
            const Message& message);
     ...
     * This method can be called on any thread.
     */
    void removeMessages(const sptr<MessageHandler>& handler);
     ...
     * This method can be called on any thread.
     */
    void removeMessages(const sptr<MessageHandler>& handler, int what);
     ...
     */
    static sptr<Looper> prepare(int opts);
     ...
     */
    static void setForThread(const sptr<Looper>& looper);

    /**
     * Returns the looper associated with the calling thread, or NULL if
     * there is not one.
     */
    static sptr<Looper> getForThread();
...
private:
    ...
    struct MessageEnvelope {
        ...
        MessageEnvelope(nsecs_t u, const sptr<MessageHandler> h,
