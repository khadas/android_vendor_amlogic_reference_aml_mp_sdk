...
// This provides primarily wptr<> weak pointer types and RefBase, which work
...
#include "AmlMpStrongPointer.h"
//#include "AmlMpTypeHelpers.h"
...
#define AML_MP_COMPARE_WEAK(_op_)                                      \
template<typename U>                                            \
...
static inline bool _wptr_compare_(T* a, U* b) {
    return comparator<typename std::common_type<T*, U*>::type>()(a, b);
}

// Use std::less and friends to avoid undefined behavior when ordering pointers
// to different objects.
#define AML_MP_COMPARE_WEAK_FUNCTIONAL(_op_, _compare_)                 \
template<typename U>                                             \
inline bool operator _op_ (const U* o) const {                   \
    return _wptr_compare_<_compare_>(m_ptr, o);                    \
}
...
public:
    ...
    typedef AmlMpRefBase basetype;

protected:
                            AmlMpRefBase();
    virtual                 ~AmlMpRefBase();
...
private:
    ...
    class weakref_impl;
                            ...
                            AmlMpRefBase(const AmlMpRefBase& o);
            AmlMpRefBase&        operator=(const AmlMpRefBase& o);
...
private:
    ...
    static void renameRefId(AmlMpRefBase* ref,
            const void* old_id, const void* new_id);
...
{
public:
    typedef typename AmlMpRefBase::weakref_type weakref_type;

    inline wptr() : m_ptr(nullptr), m_refs(nullptr) { }

    wptr(T* other);  // NOLINT(implicit)
    wptr(const wptr<T>& other);
    explicit wptr(const sptr<T>& other);
    template<typename U> wptr(U* other);  // NOLINT(implicit)
    template<typename U> wptr(const sptr<U>& other);  // NOLINT(implicit)
    template<typename U> wptr(const wptr<U>& other);  // NOLINT(implicit)
    ...
    template<typename U> wptr& operator = (const sptr<U>& other);
    ...
    AML_MP_COMPARE_WEAK(==)
    AML_MP_COMPARE_WEAK(!=)
    AML_MP_COMPARE_WEAK_FUNCTIONAL(>, std::greater)
    AML_MP_COMPARE_WEAK_FUNCTIONAL(<, std::less)
    AML_MP_COMPARE_WEAK_FUNCTIONAL(<=, std::less_equal)
    AML_MP_COMPARE_WEAK_FUNCTIONAL(>=, std::greater_equal)
    ...
    template<typename U>
    inline bool operator == (const sptr<U>& o) const {
    ...
    inline bool operator > (const wptr<U>& o) const {
        if (m_ptr == o.m_ptr) {
            return _wptr_compare_<std::greater>(m_refs, o.m_refs);
        } else {
            return _wptr_compare_<std::greater>(m_ptr, o.m_ptr);
        }
    }
    ...
    inline bool operator < (const wptr<U>& o) const {
        if (m_ptr == o.m_ptr) {
            return _wptr_compare_<std::less>(m_refs, o.m_refs);
        } else {
            return _wptr_compare_<std::less>(m_ptr, o.m_ptr);
        }
    }
    template<typename U> inline bool operator != (const wptr<U>& o) const { return !operator == (o); }
    template<typename U> inline bool operator <= (const wptr<U>& o) const { return !operator > (o); }
    template<typename U> inline bool operator >= (const wptr<U>& o) const { return !operator < (o); }
...
template <typename T>
TextOutput& operator<<(TextOutput& to, const wptr<T>& val);
...
template<typename T> template<typename U>
wptr<T>& wptr<T>::operator = (U* other)
{
    weakref_type* newRefs =
...
template<typename T> template<typename U>
wptr<T>& wptr<T>::operator = (const wptr<U>& other)
{
    weakref_type* otherRefs(other.m_refs);
...
template<typename T> template<typename U>
wptr<T>& wptr<T>::operator = (const sptr<U>& other)
{
    weakref_type* newRefs =
...
template<typename T>
void wptr<T>::set_object_and_refs(T* other, weakref_type* refs)
{
...
template <typename T>
inline TextOutput& operator<<(TextOutput& to, const wptr<T>& val)
{
...
public:
    ...
    void move_references(sptr<TYPE>* dest, sptr<TYPE> const* src, size_t n) {
        ...
        public:
            Renamer(sptr<TYPE>* d, sptr<TYPE> const* s) : d_(d), s_(s) { }
            virtual ~Renamer() { }
        };

        memmove(dest, src, n*sizeof(sptr<TYPE>));
        TYPE::renameRefs(n, Renamer(dest, src));
    }


    template<typename TYPE> static inline
    void move_references(wptr<TYPE>* dest, wptr<TYPE> const* src, size_t n) {
        ...
        public:
            Renamer(wptr<TYPE>* rd, wptr<TYPE> const* rs) : d_(rd), s_(rs) { }
            virtual ~Renamer() { }
        };

        memmove(dest, src, n*sizeof(wptr<TYPE>));
        TYPE::renameRefs(n, Renamer(dest, src));
    }
};
...
void move_forward_type(sptr<TYPE>* d, sptr<TYPE> const* s, size_t n) {
    ReferenceMover::move_references(d, s, n);
}

template<typename TYPE> inline
void move_backward_type(sptr<TYPE>* d, sptr<TYPE> const* s, size_t n) {
    ReferenceMover::move_references(d, s, n);
}

template<typename TYPE> inline
void move_forward_type(wptr<TYPE>* d, wptr<TYPE> const* s, size_t n) {
    ReferenceMover::move_references(d, s, n);
}

template<typename TYPE> inline
void move_backward_type(wptr<TYPE>* d, wptr<TYPE> const* s, size_t n) {
    ReferenceMover::move_references(d, s, n);
}
