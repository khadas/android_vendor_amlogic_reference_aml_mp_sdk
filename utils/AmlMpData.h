...
#ifndef STAGEFRIGHT_FOUNDATION_AML_MP_DATA_H_
#define STAGEFRIGHT_FOUNDATION_AML_MP_DATA_H_
...
struct HIDE _AData_copier {
     ...
     */
    template<typename Tp, typename U, typename=enable_if_T_is_same_as<sptr<Tp>>>
    inline static void assign(sptr<Tp> *data, const sptr<U> &src) {
        *data = static_cast<Tp*>(src.get());
    }

    template<typename Tp, typename U, typename=enable_if_T_is_same_as<wptr<Tp>>>
    inline static void assign(wptr<Tp> *data, const wptr<U> &src) {
        sptr<U> __tmp = src.promote();
        *data = static_cast<Tp*>(__tmp.get());
    }

    template<typename Tp, typename U, typename=enable_if_T_is_same_as<sptr<Tp>>>
    inline static void assign(sptr<Tp> *data, sptr<U> &&src) {
        sptr<U> __tmp = std::move(src); // move src out as get cannot
        *data = static_cast<Tp*>(__tmp.get());
    }
...
public:
    ...
    public:
        ...
        template<typename T>
        static constexpr Flag flagFor(sptr<T>*p) { return relaxedFlagFor(p, (T*)0); }
        template<typename T>
        static constexpr Flag flagFor(wptr<T>*p) { return relaxedFlagFor(p, (T*)0); }
